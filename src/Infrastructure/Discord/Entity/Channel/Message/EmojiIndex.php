...
enum EmojiIndex
{
    ...
    public static function TryGetName(string $code): ?string
    {
        if (\array_key_exists($code, self::Emojis)) {
    ...
    public static function replaceMessage(string $msg): string
    {
        preg_match_all('/:[a-zA-Z0-9_-]+:/', $msg, $m);
        $m = $m[0];
        foreach ($m as $value) {
            $emoji = substr(substr($value, 0, -1), 1);
            $msg = str_replace($value, self::TryGetName($emoji) ?? $value, $msg);
        }
